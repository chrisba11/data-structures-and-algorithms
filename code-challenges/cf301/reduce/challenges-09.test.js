...
const countNumberOfElements = (arr) => {
  return arr.reduce((accumulator, current) => accumulator + (current/current));
};
...
const reversedString = (str) => str.split('').reduce((accumulator, current) => current + accumulator);
...
const countNumberOfChildren = (arr) => {
  ...
  arr.forEach( char => {
    if(char.children) {
      secondArray.push(char.children.reduce((accumulator) => {
        return accumulator + 1;
      },0));
    }
  });
  return secondArray.reduce((accumulator, current) => {
