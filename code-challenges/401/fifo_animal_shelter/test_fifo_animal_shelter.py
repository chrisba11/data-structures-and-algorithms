...
def test_enqueue_one_cat():
    seattle = Animal_shelter()
    seattle.enqueue('cat', 'Fluffy')

    assert seattle.cats.peek().name == 'fluffy'


def test_enqueue_multiple_cats():
    pittsburg = Animal_shelter()
    pittsburg.enqueue('cat', 'Tom')
    pittsburg.enqueue('cat', 'JERRY')
    pittsburg.enqueue('cat', 'potato')

    assert pittsburg.cats.peek().name == 'tom'


def test_enqueue_one_dog():
    lynnwood = Animal_shelter()
    lynnwood.enqueue('dog', 'FIDO')

    assert lynnwood.dogs.peek().name == 'fido'
