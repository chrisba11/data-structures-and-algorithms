...
const count = (target, input) => {
  let count = 0;
  input.forEach(array => {
    array.map(element => element === target ? count++ : element);
  });
  return count;
};
...
const totalSum = (input) => {
  let sum = 0;
  input.forEach(array => {
    sum = sum + array.reduce((accumulator, current) => accumulator + current);
  });
  return sum;
};
...
let findMaleAndFemale = (data) => {
  ...
  data.forEach(object => {
    if (object.gender === 'male' || object.gender === 'female') {
      if (string === '') {
        string = string.concat('', object.name);
      } else {
        string = string.concat(' and ', object.name);
      }
    }
  });
  return string;
