...
const listFoods = (recipe) => {
  ...
  for (let i = 0; i < recipe.ingredients.length; i++) {
    let regex = /\ .*/gm;
    let stepOne = recipe.ingredients[i].match(regex);
    console.log(stepOne.match(regex));
  }
  return result;
};
...
const splitFoods = (recipe) => {
  ...
  let second = [];
  for (let i = 0; i < recipe.ingredients.length; i++) {
    second.push( recipe.ingredients[i].split(' '));
    let third = second[i].slice(2);
    result.push(third.join(' '));
  }
  return result;
};
...
const stepActions = (recipe) => {
  ...
  recipe.steps.forEach( step => {
    let stepOne = [];
    stepOne.push(step.split(' '));
    result.push(stepOne[0][0]);
  });
  return result;
};
